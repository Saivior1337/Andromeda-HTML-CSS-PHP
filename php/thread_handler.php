...
if (isset($_POST['submit_threadMessage']) && isset($_SESSION['logged_user']) && $_SESSION['logged_user']->email_confirm == true)
{
    $create_thread = R::dispense('threads');
    $create_thread->author = $_SESSION['logged_user']->username;
    $create_thread->date = date('d.m.Y');
    $create_thread->thread_title = htmlspecialchars($_POST['threadTitle']);
    $create_thread->thread_message = htmlspecialchars($_POST['threadMessage']);
    R::store($create_thread);
    $HTML = 
    '
...
<body>
    <section class="page_header">
        <div class="header-navigation">
            <ul class="header-ul">
                ...
                <li class="header-li">
                    <?php echo "<form method='.$POST = 'POST'.'><button type='.$submit = 'submit'.' name='.$logout = 'logout'.'>Exit <i class='.$fasolid = "'fa-solid fa-arrow-right-from-bracket icon'".'></i></button></form>"; ?>
                </li>
            </ul>
        </div>
    </section>
    ...
    ';
    $write = @file_put_contents("topics/".$create_thread->thread_title.".php", $HTML);
